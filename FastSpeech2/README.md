...
## Inference for AIPark

Default base_model path: ./output/ckpt/KDY_1

Default vocoder_model path: ./vocoder

Options:

Not support batch of --mode option
```
usage: synthesize.py [-h] [--restore_step RESTORE_STEP]
                     [--base_model BASE_MODEL] [--vocoder VOCODER]
                     [--vocoder_model VOCODER_MODEL] --mode {batch,single}
                     [--source SOURCE] [--text TEXT] [--file_path FILE_PATH]
                     [--stored_path STORED_PATH] [--speaker_id SPEAKER_ID] -p PREPROCESS_CONFIG -m MODEL_CONFIG -t TRAIN_CONFIG
                     [--pitch_control PITCH_CONTROL]
                     [--energy_control ENERGY_CONTROL]
                     [--duration_control DURATION_CONTROL] --input_mode {text,file}
```

For single sentence TTS, run
```
python synthesize.py --mode single -p config/KDY/preprocess.yaml -m config/KDY/model.yaml -t config/KDY/train.yaml --input_mode text --text "테스트입니다." --base_model output/ckpt/KDY_1/720000.pth.tar --stored_path ../wav_result
```

For sentences TTS on file, run
```
python synthesize.py --mode single -p config/KDY/preprocess.yaml -m config/KDY/model.yaml -t config/KDY/train.yaml --input_mode file --file_path kdy_tts.txt --base_model output/ckpt/KDY_1/720000.pth.tar --stored_path ../wav_result
```

Change vocoder model, run
```
python synthesize.py --mode single -p config/KDY/preprocess.yaml -m config/KDY/model.yaml -t config/KDY/train.yaml --input_mode file --file_path kdy_tts_04.txt --base_model output/ckpt/KDY_1/720000.pth.tar --vocoder_model ./vocgan/KDY_2_2_d13ed5e_8120.pt --stored_path ../wav_result
```
